  ...
  return <>
    ...
    <div className="analytics">
      ...
      <article className="panel">
        ...
        {demoTransactions.filter((item) => item[3] < 0).map((item, index) => <div className="rank" key={item[0]}><b>{index + 1}</b><span>{t(item[0])}<small>{t(item[1])}</small></span><strong>₺{formatNumber(Math.abs(item[3]), language)}</strong></div>)}
      </article>
